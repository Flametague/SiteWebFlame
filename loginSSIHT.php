...
if(isset($_POST['formulaire'])) {
  ...
  if(!empty($name)&&!empty($pws)) {
    $checkuser = $bdd->prepare("SELECT * FROM ssiht WHERE prenomnom = ? AND motsdepasse = ?");
    ...
    if ($lookforuser == 1) {
      ...
      $_SESSION['classe'] = "ssiht";
      header("Location: Ssiht.php");
    } else {
...
if(isset($_SESSION['id']) && $_SESSION['classe'] == "ssiht") {
    sleep(0.5);
    header('Location: Ssiht.php');
}
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="loginSSIHT.css?ver=1.3">
