...
if(isset($_POST['formulaire'])) {
  ...
  if(!empty($name)&&!empty($pws)) {
    $checkuser = $bdd->prepare("SELECT * FROM risc WHERE prenomnom = ? AND motsdepasse = ?");
    ...
    if ($lookforuser == 1) {
      ...
      $_SESSION['classe'] = "risc";
      header("Location: Risc.php");
    } else {
...
if(isset($_SESSION['id']) && $_SESSION['classe'] == "risc") {
    sleep(0.5);
    header('Location: Risc.php');
}
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="loginRISC.css?ver=1.3">
